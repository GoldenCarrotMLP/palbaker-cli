...
def main():
    ...
    if ACTION == "create_blend":
        ...
        # Sort files so LOD0 is prioritized over LOD1, LOD2, etc.
        psk_files.sort()
        processed_bases = set()
        
        # Loop through ALL discovered meshes to support multi-model folders (e.g., SK_Anubis and SK_Anubis_Dark)
        for psk_file in psk_files:
            psk_base = os.path.splitext(os.path.basename(psk_file))[0]
            if psk_base.lower().startswith("sk_"):
                psk_base = psk_base[3:]
                
            # Strip _LOD0, _LOD1, etc. generated by newer versions of Cue4Parse
            psk_base = re.sub(r'_LOD\d+$', '', psk_base, flags=re.IGNORECASE)
            
            # Skip redundant lower-quality LOD meshes
            if psk_base in processed_bases:
                continue
            processed_bases.add(psk_base)
                
            blend_file = os.path.join(workspace.fmodel_dir, f"{psk_base}.blend")
    ...
    if ACTION in ["push", "full"]:
        ...
        for target_dir, virtual_path in import_targets:
            blend_files = glob.glob(os.path.join(target_dir, "*.blend"))
            is_single_mesh = len(blend_files) == 1
            is_altermatic_target = "palbaker" in target_dir.replace("\\", "/").lower()
